...
function _test_exits_with_error_when_argument_count_is_wrong {
	...
	output=$(bash "${SCRIPT}" s3 /source 2>&1 || true)
...
function _test_passes_plain_path_to_aws_without_include {
	...
	output=$(LIFERAY_OVERLAY_BUCKET_NAME="test-bucket" bash "${SCRIPT}" s3 source/path dest/path 2>&1)

	[[ "${output}" == *"aws s3 sync s3://test-bucket/source/path"* ]] && [[ "${output}" != *"--include"* ]]
}

function _test_splits_glob_pattern_into_path_and_include_filter {
	local output

	output=$(LIFERAY_OVERLAY_BUCKET_NAME="test-bucket" bash "${SCRIPT}" s3 "source/path/*.jar" dest/path 2>&1)

	[[ "${output}" == *"aws s3 sync s3://test-bucket/source/path"* ]] && [[ "${output}" == *"--exclude * --include *.jar"* ]]
}
...
function _test_strips_wildcard_and_passes_include_for_wildcard_path {
	...
	[[ "${output}" == *"aws s3 sync s3://test-bucket/source/path"* ]] && [[ "${output}" == *"--exclude * --include *"* ]]
}

function _test_target_path_is_prefixed_with_temp {
	local output

	output=$(LIFERAY_OVERLAY_BUCKET_NAME="test-bucket" bash "${SCRIPT}" s3 source/path osgi/configs 2>&1)
