...
#
# Bootstrap tarballs whose packaged sources were rewritten during the current
# run. The release workflow refuses to republish a tarball version that already
# exists in storage, so every bootstrap recorded here still owes its own version
# bump. A bootstrap is only ever bumped once per run.
#
...
function main {
	...
	#
	# A version can only reach its consumers once the chart itself owns a new
	# version, and rewriting a consumer that is packaged inside another chart or
	# inside a bootstrap tarball invalidates that artifact in turn, so keep
	# propagating until nothing moves.
	#
...
#
# Runs a command that rewrites a file packaged in a bootstrap tarball and
# records the owning bootstrap when the rewrite changed the file.
#
