...
_MAXIMUM_CONVERGENCE_PASSES=16

readonly _MAXIMUM_CONVERGENCE_PASSES

_CONVERGENCE_PASSES=0

#
# Charts whose packaged content was rewritten during the current run. The
# release workflow refuses to republish a version that already exists in the
# registry, so every chart recorded here still owes its own version bump. A
# chart is only ever bumped once per run, no matter how many rewrites reach it.
#
...
function count_convergence_pass {
	_CONVERGENCE_PASSES=$((_CONVERGENCE_PASSES + 1))

	if [[ "${_CONVERGENCE_PASSES}" -gt "${_MAXIMUM_CONVERGENCE_PASSES}" ]]
	then
		echo "The versions were unable to converge after ${_MAXIMUM_CONVERGENCE_PASSES} passes." >&2
...
#
# Runs a command that rewrites a file belonging to a Helm chart and records the
# owning chart when the rewrite changed the file.
#
