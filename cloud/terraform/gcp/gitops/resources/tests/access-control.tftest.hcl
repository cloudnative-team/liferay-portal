...
run "should_allow_only_the_configured_cidrs" {
	assert {
		condition=join(",", kubernetes_manifest.argocd_gateway_authorization[0].manifest.spec.authorization.rules[0].principal.clientCIDRs) == "10.0.0.0/16,203.0.113.0/24"
		error_message="The Argo CD SecurityPolicy must allow exactly the configured CIDR blocks"
	}
	assert {
		condition=join(",", kubernetes_manifest.argo_workflows_gateway_authorization[0].manifest.spec.authorization.rules[0].principal.clientCIDRs) == "10.0.0.0/16,203.0.113.0/24"
		error_message="The Argo Workflows SecurityPolicy must allow exactly the configured CIDR blocks"
	}
	assert {
		condition=kubernetes_manifest.argocd_gateway_authorization[0].manifest.spec.authorization.rules[0].action == "Allow"
...
run "should_target_the_gateways_rather_than_the_routes" {
	...
	assert {
		condition=kubernetes_manifest.argocd_gateway_authorization[0].manifest.spec.targetRefs[0].name == "argocd-gateway"
