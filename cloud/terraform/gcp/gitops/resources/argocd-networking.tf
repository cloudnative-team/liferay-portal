...
resource "kubernetes_manifest" "argocd_gateway_security_policy" {
	...
	manifest={
		...
		metadata={
			labels=merge(
				local.common_labels,
				{
					"app.kubernetes.io/name"="${local.argocd_gateway_name}-security-policy"
				})
			name="${local.argocd_gateway_name}-security-policy"
			namespace=var.argocd_namespace
		}
		spec={
			authorization={
